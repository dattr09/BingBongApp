...
export default function ProfileScreen() {
  ...
  const pickAvatar = async () => {
    Toast.show({ type: "info", text1: "Fake pick avatar success" });
  };
  const pickCoverPhoto = async () => {
    Toast.show({ type: "info", text1: "Fake pick cover photo success" });
  };
  ...
  const renderActionButtons = () => {
    ...
    if (isFriend) {
      return (
        <View className="flex-row items-center justify-center gap-3 z-10">
          ...
          <TouchableOpacity
            onPress={() =>
              userChat && navigation.navigate("Chat", { userChat })
            }
    ...
    return (
      <View className="flex-row items-center justify-center gap-3">
        ...
        >
          <UserPlus
            color={hasSentFriendRequest ? "#374151" : "white"}
            size={18}
            strokeWidth={2.5}
          />
          <Text
            className={`font-bold ${hasSentFriendRequest ? "text-gray-700" : "text-white"}`}
          >
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white pb-6 rounded-b-3xl shadow-sm mb-4">
          ...
          <View className="relative h-60 w-full">
              ...
              <Image
                source={
                  displayProfile?.coverPhoto
                    ? {
                        uri: displayProfile.coverPhoto.startsWith("http")
                          ? displayProfile.coverPhoto
                          : `${Config.BACKEND_URL}${displayProfile.coverPhoto}`,
                      }
                    : { uri: "https://placehold.co/800x400/e2e8f0/e2e8f0.png" }
                }
            ...
            <View className="absolute -bottom-16 left-0 right-0 items-center">
              <TouchableOpacity
                onPress={isMyProfile ? pickAvatar : undefined}
                activeOpacity={0.9}
                disabled={!isMyProfile}
                className="relative"
              >
                <Image
                  source={
                    displayProfile?.avatar
                      ? {
                          uri: displayProfile.avatar.startsWith("http")
                            ? displayProfile.avatar
                            : `${Config.BACKEND_URL}${displayProfile.avatar}`,
                        }
                ...
                />
                {/* Online Status */}
                {((!isMyProfile && onlineUsers.includes(userId)) ||
                  isMyProfile) && (
            ...
            <View className="flex-row items-center gap-6 mt-4 mb-6">
              <TouchableOpacity className="items-center">
                <Text className="text-lg font-bold text-gray-900">
                  {displayProfile?.friends?.length || 0}
                </Text>
                <Text className="text-xs text-gray-500 uppercase font-semibold tracking-wide">
                  Friends
                </Text>
              </TouchableOpacity>
        ...
        >
          {["Posts", "About", "Photos", "Friends", "Videos"].map(
            (tab, index) => (
              <TouchableOpacity
                key={tab}
                className={`px-5 py-2 rounded-full ${index === 0 ? "bg-black" : "bg-white border border-gray-200"}`}
              >
                <Text
                  className={`font-semibold ${index === 0 ? "text-white" : "text-gray-600"}`}
                >
                  {tab}
                </Text>
              </TouchableOpacity>
            ...
            <View className="gap-3">
              ...
              <View className="flex-row items-center gap-3">
                <View className="bg-gray-100 p-2 rounded-full">
                  <Ellipsis size={20} color="#4b5563" />
                </View>
                <Text className="text-gray-500 italic">
                  More info about {displayProfile?.firstName}...
                </Text>
              </View>
            </View>
          </View>
            ...
            <View className="flex-row justify-between items-center mb-4">
              <View>
                ...
                <Text className="text-xs text-gray-500">
                  {displayProfile?.friends?.length || 0} connections
                </Text>
              </View>
            ...
            <View className="flex-row flex-wrap gap-2">
              {profile?.friends && profile.friends.length > 0 ? (
                Array.from({ length: 3 }).map(
                  (
                    _,
                    i // Giả lập hiển thị 3 bạn bè cho đẹp grid
                  ) => (
                    <View key={i} className="w-[31%] mb-2">
                      <TouchableOpacity>
                        <Image
                          source={{
                            uri: `https://i.pravatar.cc/300?img=${10 + i}`,
                          }}
