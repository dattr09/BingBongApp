...
export default function UserBadge({ badge, mode = "mini" }) {
  ...
  return (
    <View
      className={`flex-row items-center gap-2 border font-semibold ${isLarge ? "px-3 py-2 rounded-lg" : "px-2 py-1 rounded-md"
        }`}
      style={{
    ...
    >
      {/* Tier circle */}
      <View
        className={`items-center justify-center font-bold rounded-full ${isLarge ? "w-5 h-5" : "w-3 h-3"
          }`}
